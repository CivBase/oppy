...
class RelayCell(FixedLenCell):
    ...
    def _parseRelayHeader(self, data):
        ...
        if r.rpayload_len > defs.MAX_RPAYLOAD_LEN:
            raise BadRelayCellHeader(
                'rpayload_len {} found, but max rpayload_len is {}'.format(
                    r.rpayload_len, defs.MAX_RPAYLOAD_LEN))

        if r.cmd not in defs.RELAY_CMD_IDS:
            raise BadRelayCellHeader('Unrecognized relay cmd {}'.format(r.cmd))
    ...
    def relayHeaderRange(self):
        """Return a two-tuple indicating the (start,end) indices of this cell's
        relay header.
    ...
    def relayPayloadRange(self):
        ...
        :returns: **tuple, int** (start, end) indices of this cell's
            relay payload
        """
        start = self.relayHeaderRange()[1]
        end = start + self.rheader.rpayload_len
        return start, end

    @classmethod
    def _getSubclass(cls, header, data):
        """Uses *header* to interpret the given cell data. A cell type which will
        be appropriate for encapsulating/representing this cell data is then
    ...
    def _initSubclassMap():
        RelayCell._subclass_map = {
            defs.RELAY_BEGIN_CMD:     RelayBeginCell,
            defs.RELAY_DATA_CMD:      RelayDataCell,
            defs.RELAY_END_CMD:       RelayEndCell,
            defs.RELAY_CONNECTED_CMD: RelayConnectedCell,
            defs.RELAY_SENDME_CMD:    RelaySendMeCell,
            defs.RELAY_EXTEND_CMD:    RelayExtendCell,
            defs.RELAY_EXTENDED_CMD:  RelayExtendedCell,
            defs.RELAY_TRUNCATE_CMD:  RelayTruncateCell,
            defs.RELAY_TRUNCATED_CMD: RelayTruncatedCell,
            defs.RELAY_DROP_CMD:      RelayDropCell,
            defs.RELAY_RESOLVE_CMD:   RelayResolveCell,
            defs.RELAY_RESOLVED_CMD:  RelayResolvedCell,
            defs.RELAY_BEGIN_DIR_CMD: RelayBeginDirCell,
            defs.RELAY_EXTEND2_CMD:   RelayExtend2Cell,
            defs.RELAY_EXTENDED2_CMD: RelayExtended2Cell
        }

    @staticmethod
    def _extractRecognized(header, data):
        """Use *header* to interpret cell data; extract and return *recognized*
        field value.
    ...
    def _extractRelayCmd(header, data):
        """Use *header* to interpret the given cell data; extract and return
        *relay_cmd* field value.
    ...
    def __repr__(self):
        return '{}(header={}, rheader={}, rpayload={})'.format(
            type(self).__name__, repr(self.header), repr(self.rheader),
            repr(self.rpayload))
    ...
    class RelayHeader(object):
        ...
        def __repr__(self):
            return ('RelayCell.RelayHeader(cmd={}, recognized={}, '
                    'stream_id={}, digest={}, rpayload_len={})'.format(
                        repr(self.cmd), repr(self.recognized),
...
class RelayBeginCell(RelayCell):
    ...
    def make(circ_id, stream_id, request, flags=None, link_version=3):
        """Build and return a RelayBegin cell, filling in default values when
        possible.
        ...
        if flags is None:
            flags = [defs.BEGIN_FLAG_IPv6_OK]

        f = 0
        for flag in flags:
            if flag not in defs.RELAY_BEGIN_FLAGS:
                raise BadPayloadData(
                    'Unrecognized Relay Begin flag: {}'.format(flag))
        ...
        h = FixedLenCell.Header(
            circ_id=circ_id, cmd=defs.RELAY_CMD, link_version=link_version)

        r = RelayCell.RelayHeader(
            cmd=defs.RELAY_BEGIN_CMD,
            recognized=defs.RECOGNIZED,
            stream_id=stream_id,
            digest=defs.EMPTY_DIGEST,
    ...
    def __repr__(self):
        return 'RelayBeginCell({}, rheader={}, addr={}, flags={})'.format(
            repr(self.header), repr(self.rheader), repr(self.addr),
            repr(self.flags))
...
class RelayConnectedCell(RelayCell):
    ...
    def _parseRelayPayload(self, data):
        ...
        """
        start = self.relayPayloadRange()[0]
        offset = start
        self.addr = data[offset:offset + defs.IPv4_ADDR_LEN]
        offset += defs.IPv4_ADDR_LEN
        ...
        if self.addr == ZERO_ADDR_STR:
            ...
            self.addr = data[offset:offset + defs.IPv6_ADDR_LEN]
            offset += defs.IPv6_ADDR_LEN
    ...
    def __repr__(self):
        return ('RelayConnectedCell=({}, rheader={}, addr_type={}, addr={}, '
                'ttl={})'.format(
                    repr(self.header), repr(self.rheader), repr(self.addr_type),
                    repr(self.addr), repr(self.ttl)))
...
class RelayDataCell(RelayCell):
    ...
    def make(circ_id, stream_id, rpayload, link_version=3):
        """Construct and return a RelayData cell, using default values where
        possible.
        ...
        """
        if len(rpayload) > defs.MAX_RPAYLOAD_LEN:
            raise BadPayloadData()
...
class RelayEndCell(RelayCell):
    ...
    @staticmethod
    def make(circ_id, stream_id, reason=defs.REASON_DONE, reason_data='',
             link_version=3):
        ...
        return RelayEndCell(
            h, rheader=r, reason=reason, reason_data=reason_data)
    ...
    def _parseRelayPayload(self, data):
        ...
        if self.reason == defs.REASON_EXITPOLICY:
    ...
    def __repr__(self):
        return ('RelayBeginCell({}, rheader={}, reason={}, '
                'reason_data={})'.format(
                    repr(self.header), repr(self.rheader), repr(self.reason),
                    repr(self.reason_data)))
...
class RelayExtend2Cell(RelayCell):
    ...
    def make(circ_id, stream_id=0, nspec=None, lspecs=None,
             htype=defs.NTOR_HTYPE, hlen=defs.NTOR_HLEN, hdata='',
        ...
        if stream_id != 0:
            raise BadPayloadData('EXTEND2 cells should use stream_id=0.')

        if htype != defs.NTOR_HTYPE:
            raise BadPayloadData(
                'htype was {}, but we currently only support {} (NTor) '
                'handshakes.'.format(htype, defs.NTOR_HTYPE))

        if hlen != defs.NTOR_HLEN:
            raise BadPayloadData(
                'htype was NTor and hlen was {} but expecting {}'.format(
                    hlen, defs.NTOR_HLEN))

        if hlen != len(hdata):
            raise BadPayloadData(
                'hlen {} neq len(hdata) {}'.format(hlen, len(hdata)))
        ...
        if nspec != len(lspecs):
            raise BadPayloadData(
                'Expected {} LinkSpecifiers but found {}'.format(
                    nspec, len(lspecs)))
        ...
        r = RelayCell.RelayHeader(
            cmd=defs.RELAY_EXTEND2_CMD,
            recognized=defs.RECOGNIZED,
            stream_id=stream_id,
            digest=defs.EMPTY_DIGEST,
            rpayload_len=rpayload_len)

        return RelayExtend2Cell(
            h, rheader=r, nspec=nspec, lspecs=lspecs, htype=htype, hlen=hlen,
            hdata=hdata)
    ...
    def __repr__(self):
        return ('RelayExtend2Cell({}, rheader={}, nspec={}, lspecs={}, '
                'htype={}, hlen={}, hdata={})'.format(
                    repr(self.header), repr(self.rheader), repr(self.nspec),
                    repr(self.lspecs), repr(self.htype), repr(self.hlen),
                    repr(self.hdata)))
...
class RelayExtended2Cell(RelayCell):
    ...
    def __repr__(self):
        return 'RelayExtended2Cell({}, rheader={}, hlen={}, hdata={})'.format(
            repr(self.header), repr(self.rheader), repr(self.hlen),
            repr(self.hdata))
...
class RelaySendMeCell(RelayCell):
    ...
    def make(circ_id, stream_id=0, link_version=3):
        """Construct and return a RelaySendMeCell, using default values where
        possible.
...
class RelayTruncatedCell(RelayCell):
    ...
    def __repr__(self):
        return 'RelayTruncatedCell=({}, rheader={}, reason={})'.format(
            repr(self.header), repr(self.rheader), repr(self.reason))
