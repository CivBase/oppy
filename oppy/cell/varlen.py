...
from oppy.connection import definitions as conn_defs
...
class VarLenCell(Cell):
    ...
    def __init__(self, header, payload=None):
        ...
        if not isinstance(header, VarLenCell.Header):
            raise BadCellHeader(
                'Expected cell header type VarLenCell.Header, but received '
    ...
    def _parseHeader(self, data):
        ...
        if self.header.link_version <= 3:
            start = defs.PAYLOAD_START_V3

        else:
            start = defs.PAYLOAD_START_V4

        end = start + defs.PAYLOAD_FIELD_LEN
    ...
    def _initSubclassMap():
        VarLenCell._subclass_map = {
            defs.VERSIONS_CMD:       VersionsCell,
            defs.VPADDING_CMD:       VPaddingCell,
            defs.CERTS_CMD:          CertsCell,
            defs.AUTH_CHALLENGE_CMD: AuthChallengeCell,
            defs.AUTHENTICATE_CMD:   AuthenticateCell,
            defs.AUTHORIZE_CMD:      AuthorizeCell
        }
    ...
    class Header(object):
        ...
        def __repr__(self):
            return ('VarLenCell.Header(circ_id={}, cmd={}, payload_len={}, '
                    'link_version={})'.format(
                        self.circ_id, self.cmd, self.payload_len,
                        self.link_version))
...
class AuthChallengeCell(VarLenCell):
    ...
    def make(circ_id, challenge, methods, link_version=3):
        ...
        h = VarLenCell.Header(
            circ_id=circ_id, cmd=defs.AUTH_CHALLENGE_CMD,
            payload_len=payload_len, link_version=link_version)
    ...
    def _checkChallenge(challenge):
        if len(challenge) != CHALLENGE_LEN:
            raise BadPayloadData(
                "AuthChallengeCell must have a 'challenge' of length {}. Got "
                "challenge of length {}.".format(CHALLENGE_LEN, len(challenge)))

    @staticmethod
    def _checkNMethods(n_methods):
        if n_methods != NUM_SUPPORTED_METHODS:
            raise BadPayloadData(
                'AuthChallengeCells currently only support {} methods. Got {} '
                'methods.'.format(NUM_SUPPORTED_METHODS, n_methods))
    ...
    def _checkMethods(methods):
        for method in methods:
            if len(method) != 2:
                raise BadPayloadData(
                    'AuthChallengeCell auth methods must be 2 bytes long. '
                    'Found auth method {} bytes long.'.format(len(method)))

            if method not in SUPPORTED_METHODS:
                raise BadPayloadData(
                    'Tried to use method {}, but oppy only supports method '
                    '{}.'.format(method, SUPPORTED_METHODS))

    def __repr__(self):
        return ('AuthChallengeCell({}, challenge={}, n_methods={}, '
                'methods={})'.format(
                    repr(self.header), repr(self.challenge),
...
class CertsCell(VarLenCell):
    ...
    def make(circ_id, cert_payload_items, link_version=3):
        ...
        if num_certs > MAX_CERTS_PER_CELL:
            raise BadPayloadData(
                'CertsCell cannot have more than {} certificates per cell. '
                'Found {}.'.format(MAX_CERTS_PER_CELL, num_certs))
        ...
        h = VarLenCell.Header(
            circ_id=circ_id, cmd=defs.CERTS_CMD, payload_len=payload_len,
    ...
    def _parsePayload(self, data):
        ...
        if end - start < CERT_TYPE_LEN + CERT_LEN_LEN:
            raise BadPayloadData(
                'CertsCell payload was too few bytes to make a valid '
                'CertsCell.')

        self.num_certs = struct.unpack(
            '!B', data[offset:offset + NUM_CERT_LEN])[0]

        offset += NUM_CERT_LEN
        if self.num_certs > MAX_CERTS_PER_CELL:
            raise BadPayloadData(
                'CertsCell cannot have more than {} certificates per cell. '
                'Found {}.'.format(MAX_CERTS_PER_CELL, self.num_certs))
        ...
        try:
            ...
            raise BadPayloadData(
                'CertsCell payload was not enough bytes to construct a valid '
                'CertsCell.')

    def __repr__(self):
        return ('CertsCell(header={}, num_certs={}, '
                'cert_payload_items={})'.format(
                    repr(self.header), repr(self.num_certs),
...
class VersionsCell(VarLenCell):
    ...
    @staticmethod
    def make(versions):
        """Construct and return a VersionsCell, using default values where
        possible.
        ...
        for version in versions:
            if version not in conn_defs.SUPPORTED_LINK_PROTOCOLS:
                raise BadPayloadData(
                    'Tried to build a VersionsCell that supports versions {}, '
                    'but oppy only supports versions {}.'.format(
                        versions, conn_defs.SUPPORTED_LINK_PROTOCOLS))

        h = VarLenCell.Header(
            circ_id=0, cmd=defs.VERSIONS_CMD, payload_len=len(versions) * 2,
            link_version=3)
    ...
    def _parsePayload(self, data):
        ...
        while offset < end:
            ...
            if v not in conn_defs.KNOWN_LINK_PROTOCOLS:
                raise BadPayloadData(
                    'VersionsCell claims to support an unknown Link Protocol '
                    'version {}.'.format(v))
    ...
    def __repr__(self):
        return 'VersionsCell({}, versions={})'.format(
            repr(self.header), repr(self.versions))
...
class VPaddingCell(VarLenCell):
    ...
    def make(circ_id, padding_len, link_version=3):
        """Construct and return a VPaddingCell, using default values where
        possible.
        ...
        """
        h = VarLenCell.Header(
            circ_id=circ_id, cmd=defs.VPADDING_CMD, payload_len=padding_len,
            link_version=link_version)
