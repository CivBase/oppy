...
class LinkSpecifier(object):
    ...
    def __init__(self, relay, legacy=False):
        ...
        """
        if legacy is True:
            self.lstype = defs.LSTYPE_LEGACY
            self.lslen = defs.LSLEN_LEGACY
            ...
            if isinstance(addr, ipaddress.IPv4Address):
                self.lstype = defs.LSTYPE_IPv4
                self.lslen = defs.LSLEN_IPv4

            else:
                self.lstype = defs.LSTYPE_IPv6
                self.lslen = defs.LSLEN_IPv6
    ...
    def getBytes(self):
        """Build and construct the raw byte string represented by this link
        specifier.
...
class TLVTriple(object):
    ...
    def __init__(self, addr):
        ...
        if isinstance(addr, ipaddress.IPv4Address):
            self.addr_type = defs.IPv4_ADDR_TYPE
            self.addr_len = defs.IPv4_ADDR_LEN

        elif isinstance(addr, ipaddress.IPv6Address):
            self.addr_type = defs.IPv6_ADDR_TYPE
            self.addr_len = defs.IPv6_ADDR_LEN

        else:
            raise ValueError(
                'TLVTriple can only handle IPv4 and IPv6 type/length/value '
                'triples for now.')
    ...
    def parse(data, offset):
        ...
        """
        addr_type = struct.unpack(
            '!B', data[offset:offset + TLV_ADDR_TYPE_LEN])[0]

        offset += TLV_ADDR_TYPE_LEN

        # use addr_len for hostname types
        addr_len = struct.unpack(
            '!B', data[offset:offset + TLV_ADDR_LEN_LEN])[0]

        offset += TLV_ADDR_LEN_LEN

        if addr_type == defs.IPv4_ADDR_TYPE:
            value = data[offset:offset + defs.IPv4_ADDR_LEN]
            offset += defs.IPv4_ADDR_LEN
            value = ipaddress.ip_address(value).exploded

        elif addr_type == defs.IPv6_ADDR_TYPE:
            value = data[offset:offset + defs.IPv6_ADDR_LEN]
            offset += defs.IPv6_ADDR_LEN
            value = ipaddress.ip_address(value).exploded

        else:
            raise ValueError(
                "TLVTriple can't parse type {0} yet.".format(addr_type))
    ...
    def __repr__(self):
        return 'TLVTriple(addr={})'.format(
            repr(ipaddress.ip_address(self.value).exploded))
...
class CertsCellPayloadItem(object):
    ...
    @staticmethod
    def parse(data, offset):
        cert_type = struct.unpack(
            '!B', data[offset: offset + CERT_TYPE_LEN])[0]

        offset += CERT_TYPE_LEN
        if cert_type not in SUPPORTED_CERT_TYPES:
            raise BadPayloadData(
                'Got cert type {}, but oppy only supports cert types '
                '{}.'.format(cert_type, SUPPORTED_CERT_TYPES))
    ...
    def __repr__(self):
        return ('CertsCellPayloadItem(cert_type={}, cert_len={}, '
                'cert={})'.format(
                    repr(self.cert_type), repr(self.cert_len), repr(self.cert)))
